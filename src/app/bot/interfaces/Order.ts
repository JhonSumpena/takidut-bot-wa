...
export class Convert {
  ...
  if (!json || typeof json !== 'string' || json.trim().length === 0) {
    throw new SyntaxError('Input kosong atau bukan string valid untuk JSON');
  }

  try {
    const parsed = JSON.parse(json);
    return cast(parsed, r('IOrder'));
  } catch (err) {
    throw new SyntaxError(`Gagal parse JSON ke IOrder: ${(err as Error).message}`);
  }
}
...
const typeMap: any = {
  ...
  Location: o(
    [
      ...
      { json: 'wilayah', js: 'wilayah', typ: u(undefined, '') },
      { json: 'biaya_ongkir', js: 'biaya_ongkir', typ: u(undefined, 0) },
    ],
    false,
  ),
};
